...
useEffect(() => {
   setData([...state]);
    ...
    return  (<div className='cardOfThree'>
        <div className='container'>
        <h2 className='text-center mainHead'>{props.mainHead}</h2>
        <div className='three-cards row'>
            {data.map((el,n)=> 
                <div className={`col-md-${column} m-md-0 col-sm-12 p-0 m-sm-3 mt-5 `} key={n}>
                 ...
                 <div className='paragraph-card text-md-left'>{el.paragraph}</div>
             </div>
             </div>  
            )}    
        </div>
